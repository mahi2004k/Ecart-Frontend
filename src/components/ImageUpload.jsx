        ...
        productData.productImg.length > 0 && (
          <div className='grid grid-cols-2 gap-4 mt-3 sm:grid-cols-3'>
            {productData.productImg.map((file, idx) => {
              ...
              return (
                <Card key={idx} className='relative group overflow-hidden border border-yellow-200'>
                  <CardContent>
                    <img 
                      src={preview} 
                      alt="" 
                    ...
                    {/* 🔴 Yellow Remove Button */}
                    <button 
                      onClick={()=>removeImage(idx)} 
                      className='absolute top-1 right-1 bg-yellow-500 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition hover:bg-yellow-600'
                    >
                      <X size={14}/>
                    </button>
