...
const OrderCard = ({userOrder}) => {
  ...
  return (
    <div className='pr-20 flex flex-col gap-3 min-w-screen ml-20'>
                        ...
                        userOrder?.map((order)=>(
                            <div key={order._id} className='shadow-lg rounded-2xl p-5 border border-gray-200'>
                                ...
                                <div className='flex justify-between items-center'>
                                    ...
                                    <span className={`${order.status === "Paid" ? "bg-green-700" : order.status === "Failed" ? "bg-red-600" : "bg-orange-300"} text-white px-2 py-1 rounded-lg`}>{order.status}</span>
                                </div>
                                    ...
                                    <ul className='space-y-2'>
                                        ...
                                        order.products.map((product, index)=>(
                                            <li key={index} className='flex justify-between items-center bg-gray-50 p-3 rounded-lg'>
                                                ...
                                                <span className='p-5'>{product?.productId?._id}</span>
